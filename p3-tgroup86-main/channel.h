...
typedef struct {
    ...
    /* IMPLEMENT THIS */
    pthread_mutex_t mutex; // whenever read write or close happens, we need to acquire the lock

    // If Buffer is full, then send operations will block until space is available.
    // Else receive operations will block until data is available.
    pthread_cond_t send_cv; // condition variable to signal that a message has been sent and can be read
    pthread_cond_t receive_cv; // condition variable to signal that a message has been received and can be read
    
    bool is_closed; // to check if the channel is closed or not

    // Use to store info which thread is waiting to send or receive on this channel, so that when the channel is closed
    sem_t** send_waiting; // to keep track of the threads waiting to send on this channel
    size_t send_waiting_count; // to keep track of the number of threads waiting to
    size_t send_waiting_capacity; // to keep track of the capacity of the send_waiting array

    sem_t** receive_waiting; // to keep track of the threads waiting to receive on this channel
    size_t receive_waiting_count; // to keep track of the number of threads waiting to
    size_t receive_waiting_capacity; // to keep track of the capacity of the receive_waiting array
} chan_t;
